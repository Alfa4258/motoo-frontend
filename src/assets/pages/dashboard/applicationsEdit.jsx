...
export function ApplicationsEdit() {
    ...
    const [vmSpecified, setVmSpecified] = useState([]);
    const [gaSpecified, setGASpecified] = useState([]);
    const [companySpecified, setCompanySpecified] = useState([]);
    const [topoSpecified, setTopoSpecified] = useState([]);
    const [techSpecified, setTechSpecified] = useState([]);
    ...
    const fetchDataApplications = async () => {
        ...
        await instance.get(`/applications/${id}`).then((response) => {
            ...
            setData({
                ...response.data.data,
                first_pic: response.data.data.first_pic || '',
                virtual_machines: response.data.data.virtual_machines || [],
                group_area: response.data.data.group_area.id || '',
                product_by: response.data.data.product_by.id|| '',
    ...
    console.log('group area', groupAreas);

    console.log('Data awal: ',applications);
    console.log('data setelah berganti : ',data)
            ...
            <div className="font-bold text-lg">Edit Application Data</div>
                ...
                <div >
                        <label className="form-control w-full ">
                            ...
                            <input type="text" defaultValue={applications.name} onChange={handleChange} name="name" className="input input-bordered input-sm w-full" />
                        </label>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 py-2">
                    ...
                    {/* <div>
                        <label className="form-control w-full">
                            ...
                            >
                                <option value="" disabled>Select Group Area</option>
                                {groupAreas.map((el) => (
                    ...
                    {/* <div>
                        <label className="form-control w-full ">
                            ...
                            <select className="select select-bordered select-sm" name="product_by" value={data.product_by} onChange={handleChange}>
                                <option value={data.product_by.id}>{companies.find(company => company.id === data.product_by)?.name || 'Select Company'}</option>
                    ...
                    {[[applications.group, "group"], [applications.business_process, "businnes_process"],].map(
                        (el) => (
                            <div key={el}>
                                <label className="form-control w-full ">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 py-4">
                    {[[applications.url_prod, "url_prod"], [applications.url_dev, "url_dev"], [applications.vm_dev, "vm_dev"], [applications.vm_prod, "vm_prod"]].map(
                        (el) => (
                            <div key={el}>
                                <label className="form-control w-full ">
                                    <span className="label-text text-xs font-semibold">{el[1]}</span>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 py-4">
                    {[[applications.db_connection_path, "db_connection_path"], [applications.sap_connection_path, "sap_connection_path"], [applications.ad_connection_path, "ad_connection_path"], [applications.user_doc, "user_doc"], [applications.technical_doc, "technical_doc"], [applications.other_doc, "other_doc"]].map(
                        (el) => (
                            <div key={el}>
                                <label className="form-control w-full ">
                                    <span className="label-text text-xs font-semibold">{el[1]}</span>
                ...
                <div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        {[[applications.description,"Description"],[applications.information,"Information"]].map((el) => (
                            <div key={el}>
