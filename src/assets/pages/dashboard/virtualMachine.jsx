...
export function VirtualMachine() {
    ...
    const filteredVirtualMachines = virtualMachine.filter((vm) => {
        const { name, description, applications } = vm;
        const appMatch = applications.some((app) =>
            app.short_name.toLowerCase().includes(searchTerm.toLowerCase())
        );
                ...
                <section className="flex gap-4">
                    <div className={open ? "w-full lg:w-9/12" : "w-full"}>
                        <div className="p-2">
                            ...
                            <input
                                type="text"
                                placeholder="Search Virtual Machines or Applications"
                                value={searchTerm}
                            ...
                            {load === true ? (
                                ...
                                <>
                                    ...
                                    <table className="w-full table-x table">
                                        <thead>
                                            <tr>
                                                {["name", "description", "ip_address", "group", "applications"].map((el) => (
                                                    <td
                                                        key={el}
                                        ...
                                        <tbody>
                                            {filteredVirtualMachines.slice(startIndex, endIndex).map((vm, index) => (
                                                <tr
                                                    key={index}
                                                    onClick={() => handleOpen(vm.id)}
                                                ...
                                                >
                                                    ...
                                                    <td className="flex flex-wrap items-center">
                                                        {vm.applications.slice(0, 9).map((app) => (
                                                            app.image ? (
                                    ...
                                    <div className="flex items-center justify-between my-4">
                                        <p>Total data {filteredVirtualMachines.length ? filteredVirtualMachines.length : 0} entries</p>
                                        {filteredVirtualMachines.length > 10 && (
                                            <div className="join">
                                                <button
                            ...
                            <div role="tabpanel" className="tab-content rounded-box">
                                <div className="overflow-x-auto py-4">
                                    <table className="table table-xs table-bordered">
                                        ...
                                        <tbody>
                                            {[
                                                [virtualMachineSpecified.name, "name"],
                                                [virtualMachineSpecified.description, "description"],
                                                [virtualMachineSpecified.ip_address, "ip_address"],
                                                [virtualMachineSpecified.group, "group"],
                                            ].map((el, index) => el[0] && (
                                                <tr key={index}>
                            ...
                            <div className="modal-action">
                                <div method="dialog">
                                    <button
                                        ...
                                        onClick={() => (deleteVirtualMachine(virtualMachineSpecified.id), document.getElementById("my_modal_1").close())}
                                    >
                                        <svg
                                            xmlns="http://www.w3.org/2000/svg"
                                            fill="none"
