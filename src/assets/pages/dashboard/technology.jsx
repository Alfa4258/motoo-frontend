...
export function Technology() {
    ...
    const handleOpen = async (el) => {
        ...
        setId(el)
        console.log(el, id)
        await instance.get(`/technologies/${el}`).then((response) => {
            setTechnologySpecified(response.data.data)
    ...
    const deleteTechnology= async (id) => {
        await instance.delete(`/technologies/${id}`).then((response) => {
            setTechnology(
    ...
    const currentData = technology.slice(startIndex, endIndex);

    const totalPages = Math.ceil(technology.length / itemsPerPage);
    ...
    return (
        ...
        <section className='flex gap-4'>
            <div className={(open) ? "w-full lg:w-9/12" : "w-full"}>
                <div className=" p-2">
                <h1 className='font-bold text-xl p-2'>Technology Table</h1>
                    {load === true ?
                        ...
                        : <>
                            ...
                            <div className="flex items-center justify-between my-4">
                                ...
                                <p>Total data {technology.length ? technology.length : 0} entries</p>

                                {technology.length > 10 && (<div className="join">
                ...
                <div role="tablist" className="tabs tabs-boxed mt-4 bg-gray-200">
                    ...
                    <div className="overflow-x-auto py-4">
                        <table className="table table-xs table-bordered">
                            ...
                            <tbody>
                                {[[technologySpecified.group, "group"], [technologySpecified.name, "name"], [technologySpecified.version, "version"]].map(
                                    (el, index) => (
                                        el[0] && (
                                            <tr key={index}>
                ...
                <div className="modal-box">
                    ...
                    <div className="modal-action">
                        <div method="dialog">
                            ...
                            <button className="btn btn-success btn-sm mr-2" onClick={() => (deleteTechnology(technologySpecified.id), document.getElementById('my_modal_1').close())} ><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6">
