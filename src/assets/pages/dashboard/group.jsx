...
export function Group() {
    ...
    const [groupSpecified, setGroupSpecified] = useState([]);
    ...
    const handleOpen = async (el) => {
        ...
        setId(el);
        console.log(el, id);
        await instance.get(`/group_areas/${el}`).then((response) => {
            setGroupSpecified(response.data.data);
    ...
    const deleteGroup = async (id) => {
        await instance.delete(`/group_areas/${id}`).then((response) => {
            setGroup(
                group.filter((group) => {
                    return group.id !== id;
                })
    ...
    const currentData = group.slice(startIndex, endIndex);
    ...
    const filteredGroups = group.filter((grp) => {
        const { short_name, long_name, applications } = grp;
            ...
            {loadPage ? (
                <section className="flex gap-4">
                    <div className={open ? "w-full lg:w-9/12" : "w-full"}>
                        <div className="p-2">
                            <h1 className="font-bold text-xl p-2">Group Table</h1>
                            <input
                                type="text"
                            ...
                            {load === true ? (
                                ...
                                <>
                                    ...
                                    <table className="w-full table-x table">
                                        ...
                                        <tbody>
                                            {filteredGroups.slice(startIndex, endIndex).map((group, index) => (
                                                <tr
                                                    key={index}
                                                ...
                                                >
                                                    ...
                                                    <td>{group.long_name}</td>
                                                    <td className="flex flex-wrap items-center">
                                                        {group.applications.slice(0, 9).map((app) => (
                                                            app.image ? (
                                    ...
                                    <div className="flex items-center justify-between my-4">
                                        <p>Total data {filteredGroups.length ? filteredGroups.length : 0} entries</p>
                            ...
                            <div role="tabpanel" className="tab-content rounded-box">
                                <div className="overflow-x-auto py-4">
                                    <table className="table table-xs table-bordered">
                                        ...
                                        <tbody>
                                            {[
                                                [groupSpecified.short_name, "short_name"],
                                                [groupSpecified.long_name, "long_name"],
                                                [groupSpecified.image, "image"],
                                            ].map((el, index) => el[0] && (
                                                <tr key={index}>
                            ...
                            <div className="modal-action">
                                <div method="dialog">
                                    <button
                                        ...
                                        onClick={() => (deleteGroup(groupSpecified.id), document.getElementById("my_modal_1").close())}
                                    >
                                        <svg
                                            xmlns="http://www.w3.org/2000/svg"
                                            fill="none"
